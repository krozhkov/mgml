// https://github.com/preslavrachev/gomjml/blob/main/mjml/button_html_content_test.go
package mjml_test
...
func TestButtonWithHTMLContent(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedText:  "></a>", // In the original MJML implementation, there is no default text content value (“Button”)
			shouldNotHave: "",
		},
		{
			name:          "button without href attribute",
			mjml:          `<mjml><mj-body><mj-section><mj-column><mj-button align="left">NOTLINK</mj-button></mj-column></mj-section></mj-body></mjml>`,
			expectedText:  ">NOTLINK</p>",
			shouldNotHave: ">NOTLINK</a>",
		},
		{
			name:          "button with empty href attribute",
			mjml:          `<mjml><mj-body><mj-section><mj-column><mj-button href align="left">NOTLINK</mj-button></mj-column></mj-section></mj-body></mjml>`,
			expectedText:  ">NOTLINK</p>",
			shouldNotHave: ">NOTLINK</a>",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			html, err := mjml.MJML2Html(tt.mjml, &core.MJMLOptions{
				KeepComments:    true,
				IgnoreIncludes:  true,
